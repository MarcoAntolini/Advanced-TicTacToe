...
import { listGamesForGuestId } from "../lib/participation";
...
export const mergeGuestGames = mutation({
	...
	handler: async (ctx, args) => {
		...
		const games = await listGamesForGuestId(ctx, args.guestId);
		for (const game of games) {
			const patch: { playerX?: typeof userId; playerO?: typeof userId } = {};
			if (game.playerX === args.guestId) patch.playerX = userId;
			if (game.playerO === args.guestId) patch.playerO = userId;
			if (Object.keys(patch).length > 0) {
